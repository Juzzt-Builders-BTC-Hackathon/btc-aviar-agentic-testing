...
resumeButton.id='resume-run';resumeButton.className='secondary hidden';resumeButton.textContent='Resume run';
$('cancel-run').before(resumeButton);
resumeButton.onclick=async()=>{try{await api(`/runs/${state.selected}/resume`,{method:'POST'});await refresh();}catch(e){toast(e.message);}};
...
function openRun(demo=false, request=null) {
  ...
  $('run-scope').value=request?.scope || '';state.legacyRequirements=request?.requirements||'';
  state.prd=request?.prd_content?{name:request.prd_name,content:request.prd_content}:null;$('prd-file').value='';renderPrd();
...
function renderDetail() {
  ...
  const v2=(r.summary.pipeline_version==='v2'||r.artifacts.includes('pipeline_metadata.json')||r.artifacts.includes('planner_output.json'));
  const stages=v2?[['recon','Explore'],['prd_analyst','PRD Analyst'],['planner','Planner'],['plan_evaluator','Plan review'],['generator','Generator'],['validator','Validate'],['generation_evaluator','Generation review'],['executor','Executor'],['healer','Healer'],['final_evaluator','Final review'],['evolution','Suite changes'],['reporter','Reporter']]:[['recon','Explore'],['plan','Planner'],['coverage','Coverage'],['generate','Generator'],['validate','Validate'],['execute','Execute'],['triage','Classifier'],['heal','Healer'],['report','Quality report']];
  const visited=new Set(r.events.map(e=>e.stage));
  resumeButton.classList.toggle('hidden',!r.resume?.allowed);
  ...
  if(state.tab==='results') {
    const s=r.summary,u=s.usage||r.live_usage;
    ...
    const degraded=Object.entries(r.agent_health||{}).filter(([,h])=>h.degraded_mode);
    if(degraded.length)content+=`<div class="gap-item"><strong>Agent fallbacks used</strong>${degraded.map(([name,h])=>`<p>${esc(name)}: ${esc(h.errors.join('; ')||'Deterministic fallback')}</p>`).join('')}</div>`;
    content+=`<p class="muted">Stage: ${esc(r.stage)} · ${v2?'V2':'V1'}</p>`;
    if(r.narrative?.data?.executive_summary)content+=`<p>${esc(r.narrative.data.executive_summary)}</p>`;
    ...
    if(r.heals.length) content+=`<h3>Repair audit</h3>${r.heals.map(h=>`<div class="result-item"><strong>${esc(h.flow_id)} · ${h.verified?'Verified':'Unverified'}</strong><p>${esc(h.rationale)}</p><pre class="code">${esc(h.old_selector)} → ${esc(h.new_selector||h.candidate_selector||'no replacement')}</pre></div>`).join('')}`;
